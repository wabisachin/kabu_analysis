...
# #borokabu用。利食い指値を追加したケースの損益計算。Dataframe型のi番目の日の寄付にエントリーしてn日間保有した場合のトレード損益を算出する関数。ロスカット条件は前日の終値を下回った(上回った)時
# def calc_pl_with_open_losscut_by_yesterday_close_with_limit(df, index, holding_days, limit_range, atr_duration=20):
#     entry_price = df.loc[index, "始値"]
#     exit_price = df.loc[index+holding_days-1,"終値"]
#     close_yesterday = df.loc[index-1, "終値"]
#     high_today = df.loc[index, "高値"]
#     low_today = df.loc[index, "安値"]
#     # high_yesterday = df.loc[index-1, "高値"]
#     # low_yesterday = df.loc[index-1, "安値"]
#     atr = calc_ATR(df, index, atr_duration)
...
#                 break
#         # print("決済価格：{}".format(exit_price))
#         pl = (exit_price - entry_price) if entry_price+limit_range>high_today else limit_range
#     #ショートの場合
#     elif(entry_price < close_yesterday):
#         #損切り幅の算出
#         losscut_range = close_yesterday - entry_price
...
#                 break
#         # print("決済価格：{}".format(exit_price))
#         pl = entry_price - exit_price if entry_price - limit_range < low_today else limit_range
#     # print("----------------------------")
#     # print(df.loc[index, "日付"])
#     # print("atr:{}".format(atr))
#     # print("pl(修正前):{}円".format(pl))
